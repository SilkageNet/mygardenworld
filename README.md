...
每位系统用户（包括 admin）只能查看和操作自己的游戏账号。顶部铃铛可配置个人 Webhook，默认关闭，覆盖本人全部游戏账号；通知设置不随游戏配置复制。仅支持公网 HTTPS 通用 JSON，推送请求保护、会话失效等重要状态，默认同类异常冷却 30 分钟；失败有限重试，接收端可按 `id` 去重，投递记录保留 7 天。专用机器人格式需要接收端转换，不推送无法归属到用户的全局错误。
