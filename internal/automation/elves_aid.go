...
func flowerElvesAidOperations(s *state.State, plant *pb.PlantPolicy, now time.Time) []PlannedOp {
	...
	var ops []PlannedOp
	if planned, ok := planFlowerElvesAidReceive(s, p, now); ok {
...
func planFlowerElvesAidReceive(s *state.State, p *pb.ElvesPlantPolicy, now time.Time) (PlannedOp, bool) {
	if s == nil || p == nil {
		return PlannedOp{}, false
	}
	plantOn := elvesPlantActive(p)
	...
	goal := Goal{ID: "farm.elves_aid", Category: CategoryElves, Domain: "farm.elves_aid", Label: "花灵协助", Priority: 60}
...
func planFlowerElvesAidRequest(s *state.State, p *pb.ElvesPlantPolicy, now time.Time) (PlannedOp, bool) {
...
func PlanOneFlowerElvesAidHelp(s *state.State, p *pb.ElvesPlantPolicy, now time.Time) (PlannedOp, bool) {
	...
	goal := Goal{ID: "farm.elves_aid_help", Category: CategoryElves, Domain: "farm.elves_aid", Label: "协助好友花灵", Priority: 56}
