...
// FormatCustomerOrderRequires builds a human-readable summary of a customer
// order's requirements: direct flower needs plus flower-art items with their
// vase and recipe flowers. Returns "" if the order has no identifiable needs.
func FormatCustomerOrderRequires(s *state.State, order *state.CustomerOrder) string {
	...
	for _, req := range order.ItemRequires {
		...
		for fid, cnt := range recipeFlowerCounts(recipe) {
			flowerNames = append(flowerNames, fmt.Sprintf("%s×%d", itemLabel(fid), cnt))
		}
...
// FormatFlowerArtOpDesc formats a flower art description with art name, vase, and flowers.
// Returns "" if artID is invalid.
func FormatFlowerArtOpDesc(artID, count int32) string {
	...
	var flowerParts []string
	for fid, cnt := range recipeFlowerCounts(recipe) {
		flowerParts = append(flowerParts, fmt.Sprintf("%s×%d", itemLabel(fid), cnt))
	}
...
// withOrderReason appends the order requirement summary to a reason string.
