...
func TestStoryMainCatalogIsExactThroughTerminal(t *testing.T) {
	...
	if json.Unmarshal(rawSentinel, &sentinel) != nil || sentinel.Max != 172 {
		t.Fatalf("story sentinel=%+v", sentinel)
	}
	terminalChapter, terminalSection, ok := StoryMainTerminal()
	if !ok || terminalChapter != 173 || terminalSection != 0 {
		t.Fatalf("StoryMainTerminal=%d:%d,%t, want 173:0", terminalChapter, terminalSection, ok)
	}
	...
	last, ok := StoryMainSection(172, 5)
	if !ok || last.SectionID != 18106 || !reflect.DeepEqual(last.Cost, []ItemCount{{ItemID: 56, Count: 195}}) {
...
func TestApplyStoryMainStrictSparseAndComplete(t *testing.T) {
	...
	applyMap(t, s, map[string]any{"7": map[string]any{"101": map[string]any{"1": 173, "2": 0}}})
	story, _ = s.StoryMain()
...
func TestStoryUnlockSnapshotRequiresExactProgressAndCost(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "terminal", chapter: 172, section: 5, before: 200, nextChapter: 173, nextSection: 0, after: 5, want: true},
