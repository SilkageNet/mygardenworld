...
// DessertCatalog is the strictly validated static configuration shared by all
// 香卉甜糕 batches. Runtime identity and dates still come from namespace 23.
type DessertCatalog struct {
	TmpType         int32
	Name            string
	EnergyItemID    int32
	CurrencyItemID  int32
	...
	InitialEnergy   int32
	TaskType        int32
	CelebrityReward ItemCount
	Multipliers     []int32
	UnlockScores    []int32
	FirstDrops      []int32
	Levels          []DessertLevelInfo
}

// DessertLevelInfo is one c_actDessert cake level.
type DessertLevelInfo struct {
	Level       int32
	Name        string
	Score       int32
	ResultScore int32
...
// DessertTaskGroupInfo is one field-6 template group. TaskIndex is the
// zero-based outer-array index used by act.recv.
type DessertTaskGroupInfo struct {
	TaskIndex int32
	Tasks     []DessertTaskInfo
}

// DessertTaskInfo retains structurally valid unknown task types for
// monitoring while CatalogKnown keeps them non-actionable.
type DessertTaskInfo struct {
	Position     int32
	TaskID       int32
	TaskType     int32
	Target       int32
...
// DessertCatalogConfig returns the capture-validated static contract used by
// dessert monitoring and later safe actions. A changed resource id, mode
// multiplier, unlock threshold, task type, or celebrity reward fails closed.
func DessertCatalogConfig() (DessertCatalog, bool) {
	const (
		dessertTmpType      int32 = 5601
		dessertEnergyItemID int32 = 1342
		dessertCurrencyID   int32 = 1343
		dessertPointItemID  int32 = 1344
		dessertRemoveItemID int32 = 1345
		dessertSelectItemID int32 = 1346
		dessertRewardBoxID  int32 = 1347
		dessertTaskType     int32 = 18
	)

	actRaw, ok := StaticRow("c_act", dessertTmpType)
	if !ok {
		return DessertCatalog{}, false
	}
	var act map[string]json.RawMessage
	if json.Unmarshal(actRaw, &act) != nil {
		return DessertCatalog{}, false
	}
	actID, idOK := readStoryMainInt32(act["id"])
	var name string
	nameOK := json.Unmarshal(act["name"], &name) == nil && strings.TrimSpace(name) != ""
	scoreIDs, scoreOK := readStrictCatalogInt32List(act["scoreId"], false)
	celebrityPair, celebrityOK := readStrictCatalogInt32List(act["celebrityRwd"], false)
	if !idOK || actID != dessertTmpType || !nameOK || !scoreOK || !equalInt32Slice(scoreIDs, []int32{dessertCurrencyID}) ||
		!celebrityOK || !equalInt32Slice(celebrityPair, []int32{dessertEnergyItemID, 20}) {
		return DessertCatalog{}, false
	}

	metaRaw, ok := StaticRow("c_actDessert", -1)
	if !ok {
		return DessertCatalog{}, false
	}
	var meta map[string]json.RawMessage
	...
	metaID, metaIDOK := readStoryMainInt32(meta["id"])
	energyID, energyOK := readStoryMainInt32(meta["$item"])
	pointID, pointOK := readStoryMainInt32(meta["$point"])
	removeID, removeOK := readStoryMainInt32(meta["$item1"])
	selectID, selectOK := readStoryMainInt32(meta["$item2"])
	boxID, boxOK := readStoryMainInt32(meta["$boxCostId"])
	levelMax, levelOK := readStoryMainInt32(meta["$lvMax"])
	initial, initialOK := readStrictCatalogInt32List(meta["$first"], false)
	multipliers, multiplierOK := readStrictCatalogInt32List(meta["$rate"], false)
	unlockTail, unlockOK := readStrictCatalogInt32List(meta["$scoreNeed"], false)
	firstDrops, firstDropOK := readStrictCatalogInt32List(meta["$firstDrop"], false)
	if !metaIDOK || metaID != -1 || !energyOK || energyID != dessertEnergyItemID || !pointOK || pointID != dessertPointItemID ||
		!removeOK || removeID != dessertRemoveItemID || !selectOK || selectID != dessertSelectItemID || !boxOK || boxID != dessertRewardBoxID ||
		!levelOK || levelMax != 11 || !initialOK || !equalInt32Slice(initial, []int32{dessertEnergyItemID, 100}) ||
		!multiplierOK || !equalInt32Slice(multipliers, []int32{1, 5, 10, 25, 100}) ||
		!unlockOK || !equalInt32Slice(unlockTail, []int32{4000, 24000, 64000, 160000}) ||
		!firstDropOK || !equalInt32Slice(firstDrops, []int32{1, 1, 2, 3, 4}) {
		return DessertCatalog{}, false
	}

	for _, itemID := range []int32{dessertEnergyItemID, dessertCurrencyID, dessertPointItemID, dessertRemoveItemID, dessertSelectItemID, dessertRewardBoxID} {
		if !dessertCatalogItemValid(itemID, dessertTmpType) {
			return DessertCatalog{}, false
		}
	}
	if !dessertTaskTypeInvariant(dessertTaskType) {
		return DessertCatalog{}, false
	}

	expectedScores := []int32{0, 5, 10, 15, 25, 40, 50, 60, 80, 100, 150}
	expectedResultScores := []int32{0, 0, 0, 0, 1, 2, 4, 8, 12, 16, 20}
	expectedScales := []float64{0.5, 0.6, 0.7, 0.8, 0.9, 1, 0.6, 0.7, 0.75, 0.9, 1}
	expectedBoxRewards := []int32{0, 0, 0, 0, 0, 0, 1, 1, 2, 3, 4}
	levels := make([]DessertLevelInfo, 0, levelMax)
	for level := int32(1); level <= levelMax; level++ {
		levelRaw, exists := StaticRow("c_actDessert", level)
		if !exists {
			return DessertCatalog{}, false
		}
		var row map[string]json.RawMessage
		if json.Unmarshal(levelRaw, &row) != nil {
			return DessertCatalog{}, false
		}
		id, idOK := readStoryMainInt32(row["id"])
		num, numOK := readStoryMainInt32(row["num"])
		score, scoreOK := readStoryMainInt32(row["score"])
		resultScore, resultOK := readStoryMainInt32(row["resScore"])
		scale, scaleOK := readStrictCatalogFloat64(row["spScale"])
		var levelName string
		levelNameOK := json.Unmarshal(row["name"], &levelName) == nil && strings.TrimSpace(levelName) != ""
		if !idOK || id != level || !numOK || num != level || !scoreOK || score < 0 || !resultOK || resultScore < 0 ||
			!scaleOK || scale <= 0 || !levelNameOK {
			return DessertCatalog{}, false
		}
		var reward []ItemCount
		if rawReward, present := row["reward"]; present {
			var rewardOK bool
			reward, rewardOK = parseStoryMainCost(rawReward)
			if !rewardOK {
				return DessertCatalog{}, false
			}
			for _, item := range reward {
				if item.ItemID != dessertRewardBoxID {
					return DessertCatalog{}, false
				}
			}
		}
		idx := int(level - 1)
		if score != expectedScores[idx] || resultScore != expectedResultScores[idx] || scale != expectedScales[idx] {
			return DessertCatalog{}, false
		}
		expectedBoxes := expectedBoxRewards[idx]
		if expectedBoxes == 0 {
			if len(reward) != 0 {
				return DessertCatalog{}, false
			}
		} else if len(reward) != 1 || reward[0] != (ItemCount{ItemID: dessertRewardBoxID, Count: expectedBoxes}) {
			return DessertCatalog{}, false
		}
		levels = append(levels, DessertLevelInfo{
			Level: level, Name: strings.TrimSpace(levelName), Score: score, ResultScore: resultScore, Scale: scale,
			Reward: cloneCyclicNoteItems(reward),
		})
	}

	return DessertCatalog{
		TmpType: dessertTmpType, Name: strings.TrimSpace(name), EnergyItemID: dessertEnergyItemID,
		CurrencyItemID: dessertCurrencyID, PointItemID: dessertPointItemID, RemoveItemID: dessertRemoveItemID,
		SelectItemID: dessertSelectItemID, RewardBoxItemID: dessertRewardBoxID, InitialEnergy: 100,
		TaskType: dessertTaskType, CelebrityReward: ItemCount{ItemID: dessertEnergyItemID, Count: 20},
		Multipliers: cloneInt32s(multipliers), UnlockScores: []int32{0, 4000, 24000, 64000, 160000},
		FirstDrops: cloneInt32s(firstDrops), Levels: cloneDessertLevels(levels),
	}, true
}

// ParseDessertTemplateTaskGroups decodes namespace 23.1 template field 6.
// Explicit null and [] are valid observed-empty replacements.
func ParseDessertTemplateTaskGroups(raw json.RawMessage) ([]DessertTaskGroupInfo, bool) {
	trimmed := bytes.TrimSpace(raw)
	if bytes.Equal(trimmed, []byte("null")) {
		return nil, true
	}
	var rawGroups []json.RawMessage
	if len(trimmed) == 0 || json.Unmarshal(trimmed, &rawGroups) != nil || len(rawGroups) > math.MaxInt32 {
		return nil, false
	}
	groups := make([]DessertTaskGroupInfo, 0, len(rawGroups))
	for groupIndex, rawGroup := range rawGroups {
		var fields map[string]json.RawMessage
		if json.Unmarshal(rawGroup, &fields) != nil || fields == nil {
			return nil, false
		}
		rawTasks, present := fields["5"]
		if !present {
			return nil, false
		}
		if rawMode, present := fields["6"]; present {
			mode, ok := readStoryMainInt32(rawMode)
			if !ok || mode < 0 {
				return nil, false
			}
		}
		var rows []json.RawMessage
		if isJSONNull(rawTasks) {
			rows = nil
		} else if json.Unmarshal(rawTasks, &rows) != nil || rows == nil || len(rows) > math.MaxInt32 {
			return nil, false
		}
		group := DessertTaskGroupInfo{TaskIndex: int32(groupIndex), Tasks: make([]DessertTaskInfo, 0, len(rows))}
		seen := make(map[int32]struct{}, len(rows))
		for position, rawRow := range rows {
			var values []json.RawMessage
			if json.Unmarshal(rawRow, &values) != nil || len(values) < 5 {
				return nil, false
			}
			taskID, taskIDOK := readStoryMainInt32(values[0])
			taskType, taskTypeOK := readStoryMainInt32(values[1])
			target, targetOK := readStoryMainInt32(values[2])
			if !taskIDOK || taskID <= 0 || !taskTypeOK || taskType <= 0 || !targetOK || target <= 0 {
				return nil, false
			}
			...
			seen[taskID] = struct{}{}
			param, hasParam := int32(0), !isJSONNull(values[3])
			if hasParam {
				var paramOK bool
				param, paramOK = readStoryMainInt32(values[3])
				if !paramOK || param < 0 {
					return nil, false
				}
			}
			var rewardText string
			if json.Unmarshal(values[4], &rewardText) != nil {
				return nil, false
			}
			reward, rewardOK := parseCyclicNoteRewardText(rewardText)
			if !rewardOK || len(reward) == 0 {
				return nil, false
			}
			title, catalogKnown := dessertTaskTitle(taskType, target, param)
			group.Tasks = append(group.Tasks, DessertTaskInfo{
				Position: int32(position + 1), TaskID: taskID, TaskType: taskType, Target: target, Param: param,
				HasParam: hasParam, Title: title, Reward: cloneCyclicNoteItems(reward), CatalogKnown: catalogKnown,
			})
		}
...
func dessertTaskTitle(taskType, target, param int32) (string, bool) {
	raw, ok := StaticRow("c_task_type", taskType)
	if !ok {
		return "", false
	}
	var row map[string]json.RawMessage
	...
	codeOK := json.Unmarshal(row["code"], &code) == nil && strings.TrimSpace(code) != ""
	descriptionOK := json.Unmarshal(row["desc"], &description) == nil && strings.TrimSpace(description) != ""
	if !idOK || id != taskType || !codeOK || !descriptionOK {
		return "", false
	}
	title := strings.TrimSpace(description)
	title = strings.ReplaceAll(title, "${value}", strconv.FormatInt(int64(target), 10))
...
func dessertTaskTypeInvariant(taskType int32) bool {
	raw, ok := StaticRow("c_task_type", taskType)
	if !ok {
		return false
	}
	...
	id, idOK := readStoryMainInt32(row["id"])
	var code, description string
	return idOK && id == 18 && taskType == 18 && json.Unmarshal(row["code"], &code) == nil && code == "dailyTask" &&
		json.Unmarshal(row["desc"], &description) == nil && description == "完成每日任务${value}次"
}

func dessertCatalogItemValid(itemID, tmpType int32) bool {
	raw, ok := StaticRow("c_item", itemID)
	if !ok {
		return false
	}
	var row map[string]json.RawMessage
	...
	var name string
	nameOK := json.Unmarshal(row["name"], &name) == nil && strings.TrimSpace(name) != ""
	if !idOK || id != itemID || !nameOK {
		return false
	}
	if rawSType, present := row["sType"]; present {
		sType, ok := readStoryMainInt32(rawSType)
		return ok && sType == tmpType
	}
	// The ranking point is a normal item rather than an activity-bag item.
	return itemID == 1344
}

func readStrictCatalogInt32List(raw json.RawMessage, allowZero bool) ([]int32, bool) {
	var values []json.RawMessage
	...
	for index, rawValue := range values {
		value, ok := readStoryMainInt32(rawValue)
...
func readStrictCatalogFloat64(raw json.RawMessage) (float64, bool) {
	trimmed := bytes.TrimSpace(raw)
...
func equalInt32Slice(left, right []int32) bool {
...
func cloneDessertLevels(in []DessertLevelInfo) []DessertLevelInfo {
	if len(in) == 0 {
		return nil
	}
	out := make([]DessertLevelInfo, len(in))
	copy(out, in)
	for index := range out {
		out[index].Reward = cloneCyclicNoteItems(out[index].Reward)
	}
	return out
}
