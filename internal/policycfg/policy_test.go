...
func TestFromJSONDisplacedSessionReloginIsExplicit(t *testing.T) {
	disabledPolicy, err := FromJSON(`{"schema_version":1,"basic":{"reconnect_interval_seconds":45}}`)
	if err != nil {
		t.Fatal(err)
	}
	...
	enabledPolicy, err := FromJSON(`{"schema_version":1,"basic":{"displaced_session_relogin_enabled":true}}`)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFromJSONRequiresExactSchema(t *testing.T) {
	tests := []string{
		...
		`{"schema_version":2}`,
		`{"schema_version":1,"union":{"race":{"urgent_speedup_enabled":true}}}`,
		`{"schema_version":1,"union":{"race":{"max_task_score":19}}}`,
	}
...
func TestFromJSONPreservesCurrentRacePolicy(t *testing.T) {
	explicitOff, err := FromJSON(`{"schema_version":1,"union":{"race":{"auto_stop_on_quota_done":false,"min_task_score":17}}}`)
	if err != nil {
		t.Fatal(err)
	}
	...
	explicitOn, err := FromJSON(`{"schema_version":1,"union":{"race":{"auto_stop_on_quota_done":true}}}`)
	if err != nil {
		t.Fatal(err)
	}
